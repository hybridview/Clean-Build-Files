# # ==============================================================
# @ID       $Id: GetChildItemExtension.ps1 1560 2015-12-16 19:46:13Z ms $
# @created  2011-07-01
...
Get-ChildItem
.LINK
[Adapted from: Using Proxy Commands in PowerShell](http://get-powershell.com/post/2009/01/05/Using-Proxy-Commands-in-PowerShell.aspx)
.LINK
[Practical PowerShell: Pruning File Trees and Extending Cmdlets] (http://www.simple-talk.com/dotnet/.net-framework/practical-powershell-pruning-file-trees-and-extending-cmdlets/)
...
filter FilterSvn()
{
	...
	# The .svn directory itself must also be excluded (last clause).
	# Descendants, which are still traversed, cause "svn status" to fail with
	# a warning: line 2 is for svn 1.7; line 3 below is for svn 1.8.
	$svnFilter = ($svnStatus  -notmatch  "^[?I]") `
			-and ($svnStatus  -notmatch "is not a working copy") `
			-and ($svnStatus  -notmatch "was not found") `
